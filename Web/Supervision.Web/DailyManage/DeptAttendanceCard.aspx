                            ...
                            Ext.Ajax.request({
                                ...
                                success: function (response, opts) {
                                    ...
                                    if (json.id) {
                                        ...
                                        Ext.Ajax.request({
                                            url: 'DeptAttendanceCard.aspx?action=inigrid&start=0&limit=25&id=' + id,
                                            method: 'POST',
                                            success: function (response, opts) {
                                                var json = Ext.decode(response.responseText);
                                                var filedarray = [];
                                                ...
                                                store = new Ext.data.JsonStore({
                                                    ...
                                                    pageSize: 25,
                                                    data: json.rows,
                                                    listeners: { beforeload: function () { store.getProxy().extraParams = { UserName: Ext.getCmp('UserName').getValue(), ProjectName: Ext.getCmp("ProjectName").getValue()} } }
                                                });
                                                grid.reconfigure(store, columnarray);
                                                pgbar.bindStore(store);
                                                pgbar.doRefresh()
